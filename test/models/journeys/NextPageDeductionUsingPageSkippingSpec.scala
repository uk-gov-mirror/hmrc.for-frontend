...
class NextPageDeductionUsingPageSkippingSpec extends FlatSpec with Matchers with OptionValues {
  ...
  lazy val pageFourData = PageFour(
    true, List(SubletDetails(
      "Something", SubletPart, Some("Description"), "Reason", BigDecimal(1.0), RoughDate(None, Some(12), 1980))
    )
  )

  lazy val pageFiveData = PageFive(false, Some("name"), None, Some(Address("line1", None, Some("city"), "postcode")), LandlordConnectionTypeNone, None)
